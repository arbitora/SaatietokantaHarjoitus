...
function buildSQLquery($params)
{
	...
	if ($params)
	{
		...
		if ($params->minLampotila)
		{
			array_push($whereQuery, "ulkolampotila1 >= :minlampotila
			AND ulkolampotila2 >= :minlampotila");
		}
		if ($params->maxLampotila)
		{
			array_push($whereQuery, "ulkolampotila1 <= :maxlampotila
			AND ulkolampotila2 <= :maxlampotila");
		}
...
function getWeatherStats($params) {
	...
	$whereQuery = buildSQLquery($params); // . $whereQuery
	$startQuery = "SELECT * FROM saatilasto_harj" . $whereQuery;
	$sql = $startQuery;
    try {
        $db = getDB();
        $stmt = $db->prepare($sql);
		...
		//if ($params->minLampotila != null)
			$stmt->bindParam(':minlampotila', $params->minlampotila);
		//if ($params->maxLampotila != null)
		...
		$object = $stmt->fetchAll(PDO::FETCH_OBJ);
        $db = null;
		echo $stmt->debugDumpParams() . var_export($stmt->errorInfo()); // DEBUG
